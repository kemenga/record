...
- 📷 **拍照识别**：拍摄/选择食物照片，AI（默认 GLM-5.3-Flash，火山方舟 OpenAI 兼容接口；可通过环境变量换成 Doubao-Seed-1.6-Vision 等）识别食物种类、分类、常温/冷藏/冷冻建议保鲜天数与储存建议；一次可识别多种食物
...
- 2026-09-18 21:40–22:45：MVP（T1-T14）+ 全部 P1/P2 增强（R1-R12）完成
- 视觉模型：**GLM-5.3-Flash**（火山方舟 coding 端点，OpenAI 兼容；可通过环境变量切换 Doubao 等）
- 真实链路已验证：本地 server → glm-5.3-flash → JSON 解析 → 数据库交叉校验（实测手绘苹果图被正确识别并返回三分区保鲜天数）
- 39/39 测试 + 工程静态校验全绿；共 20+ 次提交全部推送 origin/main
