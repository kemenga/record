...
- 🤖 **AI 助手聊天**：直接对话管理冰箱——"买了鸡蛋""西红柿吃完了"即自动增删；发食物照片也能识别入库；可问营养、做法、储存问题
- 🧾 **小票批量录入**：拍购物小票一键提取所有食品，批量保存（`reasoning_effort=low` 思考力度，识别约 2-3 秒）
- 🏠 **三区储存建议**：识别结果与详情页给出"推荐冷藏7天 · 常温2天 · 冷冻30天"式建议，明确哪些常温/冷藏/冷冻
- 🥗 **营养成份**：详情页展示每 100g 热量/蛋白质/脂肪/碳水/纤维（90+ 种内置数据 + 别名归一 + 分类估算兜底）
- 🍳 **菜谱**：40 道家常菜独立菜谱页——按库存标注有/缺用料、匹配徽标、做法要点、搜索；首页"可以做的菜"优先消耗临期
- 🔓 **开封状态**：开封后按分类默认窗口（乳制品3天/熟食2天/其他7天，可选天数）重算保鲜期，卡片与详情页显示"已开封"
- 🔀 **分区转移**：冷藏↔冷冻↔常温转移时按数据库新分区建议重算保鲜期，保留转移历史
- 📦 **剩余量标记**：充足/过半/见底三档，卡片"见底"角标提醒优先吃
- 🛒 **采购清单**：手动添加 + "最近吃完"一键补货 + 勾选购买后按数据库建议一键入库
- 🧊 **冰箱清单**：按"已过期 / 即将到期 / 新鲜"分组，分区+类别筛选，批量管理（多选已食用/删除），进度条与剩余天数
- 📈 **统计面板**：在库/已食用/浪费计数、及食节省估算、未来 7 天到期预测柱状图；过期删除自动计入浪费
- 🔔 **开屏临期提醒**：进入小程序一次性提示过期/临期数量（免推送权限依赖）
- 🗃 **内置保鲜数据库**：172 种中国家庭常见食物（USDA FoodKeeper / FoodSafety.gov + 中文食安科普），无 AI 也能手动添加并自动带出保鲜期
- 🔒 **本地存储**：数据仅存本机（wx.storage），照片只用于当次识别，代理不落盘；支持剪贴板导出/导入换机
...
- 2026-09-18 21:40–22:54：MVP（T1-T14）+ P1/P2 增强（R1-R12）+ V 队列（小票 OCR / 菜谱推荐 / 开屏提醒）
- 2026-09-18 23:30–19 01:30：前端重设计合入验证（W 队列）+ 校验器加固 + 最终版冲刺（D 缺陷修复 / Y 功能队列）
- 2026-09-19 00:45–01:50：**Z 队列四功能**——三区储存建议、营养成份、菜谱完整展示、AI 助手聊天（文字/图片增删食物，真实 E2E 三场景验证）
- 视觉模型：**GLM-5.3-Flash**（火山方舟 coding 端点，`reasoning_effort=low`；可经环境变量切换）
- 真实链路已验证：识别（2.4s，防幻觉校正生效）、聊天三场景（添加/删除/图片，3-4s）
- **68/68 测试 + 工程静态校验全绿；共 55+ 次提交全部推送 origin/main**
